//! End-to-end tests that exercise the compiled `rstree` binary the way a user
//! would run it.
...
fn rstree_bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_rstree"))
}
...
fn make_tmp() -> PathBuf {
    ...
    p.push(format!("rstree-cli-test-{}-{}", std::process::id(), n));
...
fn prints_tree_for_default_directory() {
    ...
    let output = rstree_bin().current_dir(&tmp).output().unwrap();
...
fn shows_hidden_with_all_flag() {
    ...
    let output = rstree_bin().arg("-a").current_dir(&tmp).output().unwrap();
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains(".secret"));

    let output2 = rstree_bin().current_dir(&tmp).output().unwrap();
